...
namespace Unity1Week
{
    static class NativeArrayUtility
    {
		...
		public static unsafe NativeArray<T> Skip<T>(ref this NativeArray<T> array, uint length) where T : struct
        {
            var dataPointer = ((byte*)NativeArrayUnsafeUtility.GetUnsafePtr(array)) + UnsafeUtility.SizeOf<T>() * length;
